$(function() {
  ...
  recipesSortByTime(recipes, time) {
    const recipesThatMatchTime = recipes.filter(recipe => recipe.cook_time < time)
    renderRecipes(recipesThatMatchTime, $('.list'));
  }
  ...
  Recipe.prototype.renderAndAppendToElement = function(element) {
    element.append(`
        <h1 class="ui centered header">
          <a href="/recipes/${this.id}">${this.title}</a>
        </h1>
    `)
  }
    ...
    return fetch(url, request)
      ...
      .then(data => {
        ...
        for (let i = 0; i < data.length; i++) {
          recipes.push(new Recipe(data[i]));
        }
  ...
  function renderRecipes(recipes) {
    recipes.forEach(recipe => recipe.renderAndAppendToElement($('.list')));
  }
});
